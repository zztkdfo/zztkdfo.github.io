...
```
### @Getter, @Setter
Getter, Setter 자동 생성 Lombok 라이브러리<br/>
### @Entity
JPA를 사용해서 테이블과 매핑할 클래스에 @Entity 어노테이션을 필수로 붙여 사용합니다.<br/>
### @Table
엔티티와 매핑할 테이블을 지정합니다. @Table을 설정하지 않으면 엔티티명(Member)이름을 테이블 명으로 사용합니다. 따라서 Table명을 설정 하고 싶을 경우, 필자와 같이 name 속성을 사용하시길 바랍니다. uniqueConstraints 속성은 DDL 생성 시에 유니크 제약조건을 만들 수 있습니다.<br/>
### @Id
@Id 어노테이션을 사용해서 기본 키를 애플리케이션에서 직접 할당 할 수 있습니다.
JPA가 제공하는 데이터베이스 기본 키 생성 전략은 직접 할당, 자동생성(IDENTITY, SEQUENCE, TABLE)전략이 있습니다. @GeneratedValue 어노테이션을 사용해서 만들 수 있습니다.<br/>
### @Column
